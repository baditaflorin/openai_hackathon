...
class MCPGatewayService:
    """Gateway for approved tools and resources exposed to MCP clients."""
    ...
    def read_resource(
        ...
        *,
        ...
        if definition is None:
            return self._error_payload("unknown_resource", f"Unknown resource: {resource_name}", resource_name=resource_name)
    ...
    def invoke_tool(
        ...
        *,
        ...
        run_state = self._runs.setdefault(
            run_key,
            AgentRunState(run_id=run_key, goal=f"{tool_name} invocation", correlation_id=correlation_id),
        )
        ...
        if normalized_mode == DRY_RUN_MODE:
            ...
            output = self._execute_tool(invocation, definition) if self._tool_executor is not None else {
                "dry_run": True,
                "tool_name": tool_name,
    ...
    def _default_resources(self) -> tuple[ResourceDefinition, ...]:
        return (
            ResourceDefinition(
                ...
                description="Resolved runtime settings and runtime status summary.",
                scope="read",
                output_schema={"type": "object"},
            ...
            ResourceDefinition(
                ...
                description="Summary of records and their current statuses.",
                scope="read",
                output_schema={"type": "object"},
            ...
            ResourceDefinition(
                ...
                description="Recent prompt-run metadata.",
                scope="read",
                output_schema={"type": "object"},
            ...
            ResourceDefinition(
                name="publish.status",
                description="Read-only publishing status summary.",
                scope="publish",
                output_schema={"type": "object"},
                tags=("publish", "status"),
            ),
    ...
    def _default_resource_payload(self, definition: ResourceDefinition, arguments: Mapping[str, Any]) -> dict[str, Any]:
        return {
